...
"""Pre-flight checks for an optimize_anything run (gepa package) — fail fast before a long job.
    ...
    python preflight.py --engine autoresearch  # also checks the Codex adapter + jq
...
def main() -> int:
    ...
    ap.add_argument(
        "--no-sandbox",
        action="store_true",
        help="skip the bwrap requirement when the run sets sandbox=False",
    )
    ap.add_argument(
        "--test-lm",
        action="store_true",
    ...
    if a.engine in ("autoresearch", "meta_harness"):
        ...
        check(
            f"Codex adapter `claude` on PATH (required by {a.engine})",
            bool(cli),
            "prepend this skill's scripts directory to PATH",
        )
        check("`codex` CLI on PATH", bool(codex), "install and authenticate Codex CLI")
        check(
            "CODEX_ADAPTER_STATE_DIR is set",
            bool(os.environ.get("CODEX_ADAPTER_STATE_DIR")),
        ...
        if sys.platform.startswith("linux") and not a.no_sandbox:
            check(
                "`bwrap` on PATH (default sandbox=True jails Codex with bubblewrap)",
